...
async def support_message(message: Message):
    ...
    if not state or state.get("mode") != "support":
        return
    ...
    text = (
        ...
        f"💬 {message.text}\n\n"
        "✏️ Ответить:\n"
        "<code>/user_reply ID сообщение</code>"
    )
...
async def support_start_cb(callback: CallbackQuery):
    user_states[callback.from_user.id] = {"mode": "support"}
...
async def support_start(message: Message):
    ...
    user_states[message.from_user.id] = {"mode": "support"}
