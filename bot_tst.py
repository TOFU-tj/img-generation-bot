...
async def show_main_menu(message_or_callback):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🖼️ Создать картинку", callback_data="select_mode:txt2img")],
        [InlineKeyboardButton(text="📷 Редактировать фото", callback_data="select_mode:img2img")],
        [InlineKeyboardButton(text="🎬 Сгенерировать видео", callback_data="select_mode:video")],
...
@router.callback_query(
    F.data.startswith("select_mode:") & ~F.data.endswith(":video")
)
...
@router.callback_query(F.data == "select_mode:video")
async def select_video_mode(callback: CallbackQuery):
    user_id = callback.from_user.id
    user_states[user_id] = {"mode": "video"}

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⏱ 5 секунд", callback_data="video_duration:5")],
        [InlineKeyboardButton(text="⏱ 10 секунд", callback_data="video_duration:10")],
        [InlineKeyboardButton(text="↩️ Назад", callback_data="back_to_modes")]
    ])
...
async def set_video_duration(callback: CallbackQuery):
    user_id = callback.from_user.id
    ...
    await callback.message.answer(
        "🖼 <b>Теперь отправьте стартовое изображение</b>\n"
        "Это будет первый кадр видео."
    )
    await callback.answer()

@router.message(F.photo)
async def handle_video_start_image(message: Message):
    user_id = message.from_user.id
    state = user_states.get(user_id)

    if not state or state.get("mode") != "video" or "duration" not in state:
        return

    file = await bot.get_file(message.photo[-1].file_id)
    url = f"https://api.telegram.org/file/bot{TELEGRAM_TOKEN}/{file.file_path}"

    state["start_image"] = url

    await message.answer(
        "✍️ Отлично!\n\nТеперь напишите описание видео (prompt)."
    )
...
async def generate_video(message: Message):
    ...
    if not gen_type:
        await message.answer("❌ Нет генераций. Используйте /banans")
        return
...
async def handle_photo(message: Message):
    ...
    if not state:
        return
            ...
            # 🎬 VIDEO
    if state.get("mode") == "video" and "duration" in state:
        file = await bot.get_file(message.photo[-1].file_id)
        url = f"https://api.telegram.org/file/bot{TELEGRAM_TOKEN}/{file.file_path}"
        state["start_image"] = url
